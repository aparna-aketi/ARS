...
parser.add_argument('--alpha', default=0.01, type=float,     help='parameter alpha that controls non-iidness -- lower the alpha higher the non-iidness')
...
parser.add_argument('--growth_rate', default=1.0, type=float, help='growth rate used for averaging rate scheduler')
parser.add_argument('--momentum', default=0.9, type=float, metavar='M', help='momentum')
parser.add_argument('--nesterov', action='store_true', )
parser.add_argument('--qgm', action='store_true', )
...
def run(rank, size):
    ...
    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, gamma = 0.1, milestones=[int(args.epochs*0.5), int(args.epochs*0.6)])
...
def train(train_loader, model, criterion, optimizer, epoch, device):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        amt_data_transfer        = model.transfer_params(optimizer.param_groups[0]['lr'], i, epoch)
        data_transferred        += amt_data_transfer
         # do global update (gossip average step)
        model.gossip_averaging()
        #activate qgm
        if args.qgm:
            model.update_momentum(optimizer)
