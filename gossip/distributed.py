...
class GossipDataParallel(Module):
    ...
    def __init__(self, module, device_ids=None, rank=None, world_size=None,
                 ...
                 verbose=False, use_streams=False, gamma=1.0, growth_rate = 1.0):
        ...
        self.one_tensor     = torch.ones(1, device=comm_device).type(first_param_dtype)
        self.averaging_rate = self.one_tensor*gamma
        self.beta2          = self.one_tensor*0.999
        ...
        for p in module.parameters():
            ...
            self.avg_err_sq.append(torch.zeros_like(cp)) 
    ...
    def ar_scheduler(self, epoch):
        self.averaging_rate.data = min(self.one_tensor.data, self.averaging_rate.data*self.growth_rate) #exponential scheduler
        # scale = 0.08+(0.46*(1-math.cos((math.pi*epoch)/299))) #cosine scheduler
        # self.averaging_rate.data.copy_(self.one_tensor.data*scale) #cosine scheduler
        if self.dist_config['rank']==0: print("Current averaging rate: ", self.averaging_rate)
    ...
    def transfer_params(self, lr=0.1, iteration=0, epoch=0):
    ...
    def gossip_averaging(self):
        ...
        for p, r in zip(self.module.parameters(), self.gossip_device_buffer):
            ...
            p.data.add_(r.data, alpha = float(self.averaging_rate))  
        return

        
    
    def update_momentum(self, optimizer):
        for param, r in zip(self.module.parameters(), self.gossip_device_buffer):
            optimizer.state[param]['momentum_buffer'].data.add_(r.data, alpha=-float(self.averaging_rate)*(1.0/self.lr))
        return
